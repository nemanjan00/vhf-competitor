...
| 17 | Ground station tier: real-time **station core** — channelization, demod, skimming, recording, **NPU AI noise cancelling**; hosts the physical operator I/O (knobs, mic + PTT, paddle) and a **built-in Ethernet switch** (dedicated point-to-point port to mast, switched ports for terminal laptops) | **considering** | Masthead FPGA stays near-stock (safety jobs only); terminals are pure-software thin clients, multi-op = plug in another laptop; NPU hardware and NR domain (audio vs IQ) open. See [04-dsp-partitioning.md](04-dsp-partitioning.md) |
